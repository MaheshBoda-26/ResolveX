...
import { useTrace, useAgentRunTrace, useConversationTraces } from '@/shared/hooks/useTrace';
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            ...
            aria-label={label}
          >
            {copied ? <Check className="h-3 w-3 text-success-default" /> : <Copy className="h-3 w-3" />}
...
function VerificationBadge({ status, onClick }: { status: 'pending' | 'passed' | 'failed'; onClick?: () => void }) {
  const icons = {
...
function HandoffCard({ handoff }: { handoff: { id: string; reason: string; evidence: Record<string, unknown>; recommendedAction: string; status: string; createdAt: string } }) {
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="mt-2 text-body text-text-secondary">{handoff.reason}</p>
            <p className="mt-1 text-body text-text-primary font-medium">Recommended: {handoff.recommendedAction}</p>
          </div>
          <button
        ...
        {expanded && (
          <div className="mt-4 space-y-3 border-t border-warning-default/30 pt-4">
            <div>
              <p className="text-small font-medium text-text-secondary mb-1">Evidence</p>
              <div className="flex items-center justify-between">
                ...
                <CopyButton value={formatJson(handoff.evidence)} label="Copy evidence" />
              </div>
            </div>
...
function TraceEventItem({ event, index, total }: { event: { id: string; timestamp: string; type: string; label: string; data: Record<string, unknown>; status: TraceEventStatus }; index: number; total: number }) {
  const [expanded, setExpanded] = useState(false);
  const EventIcon = eventTypeIcons[event.type as keyof typeof eventTypeIcons] ?? Zap;
  const eventLabel = eventTypeLabels[event.type as keyof typeof eventTypeLabels] ?? event.type;
      ...
      <CardContent className="p-0">
        <div className="flex items-start gap-4 p-4">
          <div className="flex flex-col items-center flex-shrink-0">
            ...
            >
              <EventIcon className={cn('h-4 w-4', statusColors[event.status as keyof typeof statusColors] ?? 'text-text-muted')} />
            </div>
            {index < total - 1 && (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <span className="text-body-medium font-medium">{eventLabel}</span>
              </div>
              <Badge variant={statusVariant as 'primary' | 'secondary' | 'default' | 'error' | 'success' | 'warning' | 'info'}>{event.status}</Badge>
              {(typeof event.data?.duration === 'number' || typeof event.data?.duration === 'string') && (
              ...
              <div className="mt-3">
                <Collapsible open={expanded} onOpenChange={setExpanded}>
                  ...
                  <CollapsibleContent className="mt-3 space-y-3 border-t border-border-default pt-3">
                    <div className="flex items-center justify-between">
                      <p className="text-small font-medium text-text-secondary">Data</p>
                      <CopyButton value={formatJson(event.data)} label="Copy JSON" />
                    </div>
...
function AgentTraceItem({ trace, index, total }: { trace: AgentTrace; index: number; total: number }) {
  const [expanded, setExpanded] = useState(false);
      ...
      <CardContent className="p-0">
        <div className="flex items-start gap-4 p-4">
          <div className="flex flex-col items-center flex-shrink-0">
            ...
            >
              <StatusIcon className={cn('h-4 w-4', statusColors[trace.status as keyof typeof statusColors])} />
            </div>
            {index < total - 1 && (
              <div className="w-px h-full bg-border-default mt-1" />
            )}
                ...
                <Collapsible open={expanded} onOpenChange={setExpanded}>
                  ...
                  <CollapsibleContent className="mt-3 space-y-3 border-t border-border-default pt-3">
                    {trace.input !== undefined && (
                      <div className="flex items-center justify-between">
                        <p className="text-small font-medium text-text-secondary">Input</p>
                        <CopyButton value={formatJson(trace.input)} label="Copy input" />
                      </div>
                    )}
                    {trace.input !== undefined && (
                      <pre className="bg-secondary-soft dark:bg-secondary-default p-3 rounded-lg overflow-x-auto text-trace-mono font-mono text-text-primary">
                    ...
                    {trace.output !== undefined && (
                      <div className="flex items-center justify-between">
                        <p className="text-small font-medium text-text-secondary">Output</p>
                        <CopyButton value={formatJson(trace.output)} label="Copy output" />
                      </div>
                    )}
                    {trace.output !== undefined && (
                      <pre className="bg-secondary-soft dark:bg-secondary-default p-3 rounded-lg overflow-x-auto text-trace-mono font-mono text-text-primary">
...
export function TraceTimeline({ runId, conversationId }: TraceTimelineProps) {
  ...
  const hasRunning = displayTraces.some((t) => t.status === 'running' || t.status === 'pending');
  const overallStatus = displayTraces.some((t) => t.status === 'failed') ? 'failed'
    : displayTraces.some((t) => t.status === 'escalated') ? 'escalated'
    : hasRunning ? 'running' : 'success';
    ...
    <div className="h-full flex flex-col">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            ...
            <p className="text-body text-text-secondary mt-1">
              ...
              <span className={cn('ml-1 font-medium', statusColors[overallStatus as keyof typeof statusColors] ?? '')}>
                {hasRunning ? 'Running...' : overallStatus.charAt(0).toUpperCase() + overallStatus.slice(1)}
              </span>
            </p>
          </div>
          {agentRunTrace && (
            <div className="flex items-center gap-2">
              <Badge variant={agentRunTrace.status === 'running' ? 'info' : agentRunTrace.status === 'completed' ? 'success' : 'error'}>
      ...
      <ScrollArea className="flex-1 p-4 space-y-3">
        {displayTraces.map((trace, index) => (
          <AgentTraceItem key={trace.id} trace={trace} index={index} total={displayTraces.length} />
        ))}

        {agentRunTrace?.events && agentRunTrace.events.length > 0 && (
          <>
            <Separator className="my-4" />
            <h3 className="text-body-medium font-semibold text-text-secondary px-2 mb-2">Detailed Events</h3>
            {agentRunTrace.events.map((event, index) => (
              <TraceEventItem key={event.id} event={event} index={index} total={agentRunTrace.events.length} />
            ))}
          </>
        )}
        ...
        {agentRunTrace?.handoffs && agentRunTrace.handoffs.length > 0 && (
          <>
            <Separator className="my-4" />
            <h3 className="text-body-medium font-semibold text-text-secondary px-2 mb-2">Handoffs</h3>
        ...
        {agentRunTrace?.verifications && agentRunTrace.verifications.length > 0 && (
          <>
            <Separator className="my-4" />
            <h3 className="text-body-medium font-semibold text-text-secondary px-2 mb-2">Verifications</h3>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-body-medium font-medium capitalize">{verification.actionType}</span>
                      </div>
