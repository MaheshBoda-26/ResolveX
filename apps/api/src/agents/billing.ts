...
export function detectDuplicateCharges(
  ...
  if (targetAmount !== undefined) {
    return candidates.filter(
      (t) => t.amount === targetAmount && t.invoiceId !== targetInvoiceId,
    );
  }
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        for (let j = i + 1; j < list.length; j++) {
          ...
          const diffMs = Math.abs(
            new Date(tx1.chargedAt).getTime() -
              new Date(tx2.chargedAt).getTime(),
          );
...
export async function processBillingTask(
  task: BillingTask,
  ...
  if (task.type === "duplicate_charge") {
    ...
    if (duplicates.length === 0) {
      ...
      evidence.push(
        `Duplicate charge detected: ${duplicates.length} transactions with amount $${refundAmount} within 24h`,
      );
      evidence.push(
        `Invoice IDs: ${duplicates.map((d) => d.invoiceId).join(", ")}`,
      );
  ...
  const risk =
    refundAmount !== undefined && refundAmount > 500
      ? "high"
      : refundAmount !== undefined && refundAmount > 50
        ? "medium"
        : "low";
