...
function mapPriority(priority: string): Task["priority"] {
  ...
  if (lower === "high" || lower === "urgent" || lower === "critical")
    return "high";
  if (lower === "low") return "low";
  return "normal";
}

export async function callFreshworksTriage(
  message: string,
): Promise<FreshworksTriageResponse | null> {
  return withTracing("callFreshworksTriage", async (span) => {
    span.setAttribute("message.length", message.length);

    const domain = process.env["FRESHWORKS_DOMAIN"] ?? FRESHWORKS_DOMAIN;
    const apiKey = process.env["FRESHWORKS_API_KEY"] ?? FRESHWORKS_API_KEY;
    const studioUrl =
      process.env["FRESHWORKS_AGENT_STUDIO_URL"] ?? FRESHWORKS_AGENT_STUDIO_URL;

    if (!domain || !apiKey || !studioUrl) {
      addSpanEvent("credentials_not_configured");
      console.warn(
        "Freshworks credentials not configured, skipping intent detection",
      );
      return null;
    }

    try {
      const response = await fetch(`${studioUrl}/api/triage`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        addSpanEvent("api_error", {
          status: response.status,
          statusText: response.statusText,
        });
        console.error(
          `Freshworks API error: ${response.status} ${response.statusText}`,
        );
        return null;
      }
...
function extractTargetPlan(message: string): string | undefined {
  ...
  for (const plan of plans) {
    if (lower.includes(plan)) {
      ...
      const upgradePattern = new RegExp(
        `(?:upgrade|change|switch|move).*\\b${plan}\\b`,
        "i",
      );
      const planPattern = new RegExp(`\\bplan\\s+is\\s+${plan}\\b`, "i");
      const toPattern = new RegExp(`\\bto\\s+${plan}\\b`, "i");
      if (
        upgradePattern.test(lower) ||
        planPattern.test(lower) ||
        toPattern.test(lower)
      ) {
        return plan;
      }
    }
...
export async function triageMessage(
  ...
  return withTracing("triageMessage", async (span) => {
    ...
    logAgentOperation(logger, "triageMessage", "started", {
    ...
    if (freshworksResult) {
      addSpanEvent("freshworks_result_received", {
        intentCount: freshworksResult.intents.length,
        taskCount: freshworksResult.tasks.length,
      });
      const result = {
