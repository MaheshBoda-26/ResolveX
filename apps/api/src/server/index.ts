...
export async function buildServer(): Promise<FastifyInstance> {
  ...
  server.addHook('onRequest', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    requestLogger.info({ type: 'http_request_start' }, `Request started: ${request.method} ${request.url}`);
  });

  server.addHook('onResponse', async (request: FastifyRequest, reply: FastifyReply) => {
    const durationMs = Date.now() - (request.headers['x-request-start'] ? parseInt(request.headers['x-request-start'] as string) : Date.now());
    logRequest(request.log, request.method, request.url, reply.statusCode, durationMs);
  });
  ...
  server.setErrorHandler((error: unknown, request: FastifyRequest, reply: FastifyReply) => {
    logError(request.log, error instanceof Error ? error : new Error(String(error)));
...
const start = async () => {
  try {
    ...
    logError(logger, err instanceof Error ? err : new Error(String(err)), { type: 'startup_error' });
    process.exit(1);
  }
};
