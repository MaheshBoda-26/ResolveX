...
export const billingRoutes: FastifyPluginAsync = async (app) => {
  app.post('/billing', {
    ...
    async handler(request, reply) {
      ...
      const conversation = await createConversation(body.payload.customerId, 'chat');
      ...
      try {
        if (body.type === 'duplicate_charge' || body.type === 'refund_inquiry') {
          toolCallId = (await createToolCall({
          ...
          await updateToolCallResult(toolCallId, { customerId: body.payload.customerId }, 'success', 0);
        }

        const decision = await processBillingTask(body);

        await updateAgentRunStatus(agentRun.id, 'completed', new Date(), decision);
