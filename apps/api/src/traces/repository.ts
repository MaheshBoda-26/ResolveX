...
import { agentRuns, toolCalls, verifications, handoffs } from "../db/schema";
...
export async function getAgentRunsByConversation(
...
export async function getToolCallsByAgentRun(
  agentRunId: string,
): Promise<ToolCall[]> {
  return db
    .select()
...
export async function updateToolCallResult(
  ...
  const updateData: Partial<ToolCall> = {
...
export async function getVerificationsByConversation(
  conversationId: string,
): Promise<Verification[]> {
  return db
    .select()
...
export async function updateVerificationStatus(
  ...
  if (observedState !== undefined)
    updateData.observedState = observedState as Record<string, unknown>;
  const [verification] = await db
...
export async function getAgentRunWithDetails(
  runId: string,
